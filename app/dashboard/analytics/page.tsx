...
export default function Analytics() {
    const [heatmapData, setHeatmapData] = useState<number[]>(Array(70).fill(0));
    ...
    useEffect(() => {
        async function loadData() {
            ...
            if (res.success && res.data) {
                const scores = Array(70).fill(0);
                res.data.forEach((entry: any, i: number) => {
                    if (i < 70) scores[i] = entry.habitScore || 0;
                });
                
                setHeatmapData(scores.reverse());
                
                setStats({
                    totalDeepWork: res.stats?.totalDeepWork || "0h",
                    habitScore: res.stats?.habitScore || "0%",
                    tasksCompleted: res.stats?.tasksCompleted || "0",
                    streak: "1 Day" // Mocked streak for now
                });
            }
            setLoading(false);
        }
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            {loading ? (
                <div className="p-10 font-bold text-center text-slate-500 flex flex-col items-center gap-4 mt-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        { label: "Deep Work Total", value: stats.totalDeepWork, sub: "Active Focus", icon: Activity, color: "text-blue-500" },
                        { label: "Habit Score", value: stats.habitScore, sub: "Consistency", icon: Target, color: "text-purple-500" },
                        { label: "Current Streak", value: stats.streak, sub: "Keep it up", icon: Flame, color: "text-orange-500" },
                        { label: "Tasks Completed", value: stats.tasksCompleted, sub: "Total Done", icon: CheckCircle2, color: "text-emerald-500" },
                    ].map((stat, i) => (
                        <motion.div
                            key={i}
